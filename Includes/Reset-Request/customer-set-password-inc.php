...
if(isset($_POST["customer-reset-password-submit"])){
    ...
    if(empty($cust_pwd) || empty($cust_pwd_cnf)){
        header("Location: ../../customer-set-password.php?selector=".$selector."&validator=".$validator."&newpwd=empty");
        exit();   
    }
    elseif($cust_pwd > 8){
        header("Location: ../../customer-set-password.php?selector=".$selector."&validator=".$validator."&newpwd=pwdlength");
        exit();
    }   
    elseif($cust_pwd != $cust_pwd_cnf){
        header("Location: ../../customer-set-password.php?selector=".$selector."&validator=".$validator."&newpwd=pwdnotsame");
        exit();
    }   
